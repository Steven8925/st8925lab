# Project 06 — 重建規格 / Rebuild Prompt

讀完本檔即可重建 `project-06/index.html`。本頁**依賴**根目錄的共用檔案
（`config.js`、`shared/wordmark.css`、`shared/wordmark.js`），其規格在
根目錄 [`../PROMPT.md`](../PROMPT.md)，此處不重複——只講本頁**自己**的部分。

Reading this file alone reproduces `project-06/index.html`. This page
...
**檔案 File**: `project-06/index.html`

**識別碼 Identity**: `const MY_ID = '06';` — 硬寫在本頁 `<script>` 內的唯一一行
...
2. `<main>` 置中內容 centred content:
   ...
   - `#lead`（`<p>`）— 文字內容：`this is "PROJECT 06" home page.`
   - `#title`（`<h1>`）— 文字內容：`Welcome to "PROJECT 06"`
...
這是一個**可獨立編輯**的頁面——直接修改 `project-06/index.html` 的
...
`project-06/index.html` directly; it will not affect the other five
