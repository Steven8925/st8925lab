...
Inlines config.js, geodata.js, shared/wordmark.js and app.js into
index.html (plus shared/wordmark.css as a <style>), and inlines the
project page as a hidden overlay so clicking a project works with no
server and no second file. 打包為單一檔案，無需伺服器即可驗收。

The overlay exists because a standalone file has no sibling project-XX/
folders to navigate to. On the real site the click navigates normally;
here it reveals the overlay instead, which is why PROJECT_URL is left
untouched and only the navigation call is swapped.
單檔版沒有可導向的 project-XX/ 資料夾，故改以覆蓋層呈現；PROJECT_URL
不動，只替換導向動作本身。
"""
...
wmcss = (d / 'shared' / 'wordmark.css').read_text(encoding='utf-8')
wmjs  = (d / 'shared' / 'wordmark.js').read_text(encoding='utf-8')
...
html = html.replace('<link rel="stylesheet" href="shared/wordmark.css">',
                     '<style>\n' + wmcss + '\n</style>')
html = html.replace('<script src="config.js"></script>',  '<script>\n' + cfg + '\n</script>')
html = html.replace('<script src="geodata.js"></script>', '<script>\n' + geo + '\n</script>')
html = html.replace('<script src="shared/wordmark.js"></script>',
                     '<script>\n' + wmjs + '\n</script>')
html = html.replace('<script src="app.js"></script>',     '<script>\n' + app + '\n</script>')
html = html.replace('</body>', overlay + '\n</body>')

for tag in ('config.js', 'geodata.js', 'app.js', 'shared/wordmark.js'):
    if f'src="{tag}"' in html:
        sys.exit(f'ERROR: {tag} was not inlined.')
if 'href="shared/wordmark.css"' in html:
    sys.exit('ERROR: shared/wordmark.css was not inlined.')
