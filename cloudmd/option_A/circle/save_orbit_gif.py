...
# 畫原子核 (中間紅色球體)
ax.scatter([0], [0], [0], color='#ef4444', s=280, edgecolors='#fca5a5', linewidth=2, zorder=10)
...
for angle in orbit_angles:
    ...
    z = y0 * np.sin(tilt)
    ax.plot(x, y, z, color='#38bdf8', linestyle='-', alpha=0.45, linewidth=1.5)

# 動態電子與尾跡物件
electrons = [ax.plot([], [], [], 'o', color='#38bdf8', markersize=9, markeredgecolor='#ffffff', markeredgewidth=1.5)[0] for _ in range(num_orbits)]
trails = [ax.plot([], [], [], color='#38bdf8', alpha=0.75, linewidth=2.5)[0] for _ in range(num_orbits)]
...
ani = animation.FuncAnimation(fig, update, frames=frames_count, init_func=init, blit=True)
ax.set_title("3D Rutherford Atomic Orbit Simulation", color='#f8fafc', fontsize=14, pad=12, fontweight='bold')

output_gif_path = "c:/Users/win87/Desktop/個人網站/circle/electron_orbit.gif"
writer = animation.PillowWriter(fps=30)
ani.save(output_gif_path, writer=writer)
plt.close(fig)
print("3D GIF animation saved successfully!")
