...
// 站名亦滅／逐字透鏡的常數已移至 shared/wordmark.js（全站共用元件）。
// Wordmark pulse and per-glyph lens constants now live in
// shared/wordmark.js (a site-wide shared component).
...
const BURST_PARTICLES    = 400;   // 粒子數 / particle count (400)
const BURST_SPEED_MIN    = 560;   // 粒子初速下限 px/s（再加大100%） / min initial speed (8x)
const BURST_SPEED_MAX    = 2400;  // 粒子初速上限 px/s（再加大100%） / max initial speed (8x)
const BURST_DAMPING      = 0.945; // 每幀阻尼，星雲感來源 / per-frame damping
const BURST_RING_MAX     = 960;   // 衝擊波最大半徑 px（再加大100%） / max shockwave radius (8x)
const BURST_FLASH_SCALE  = 40.0;  // 核心閃光倍率（再加大100%） / core flash multiplier (8x)
const BURST_PARTICLE_R   = 19.2;  // 粒子半徑 px（再加大100%） / particle radius (8x)
...
const BURST_CLOUDS       = 16;    // 瀰漫雲團數 / diffuse cloud blobs
const BURST_CLOUD_R      = 624;   // 雲團最大半徑 px（再加大100%） / max cloud radius (8x)
const BURST_GLOW_MULT    = 4.5;   // 粒子光暈倍率 / per-particle glow radius
const BURST_SPARKS       = 48;    // 十字星芒數 / cross-flare sparks
...
// SITE_NAME now lives in config.js — every project-XX page needs it for the
// shared wordmark too, and config.js is already the file all pages load.
// SITE_NAME 已移至 config.js — 每個 project-XX 頁面也需要它來繪製共用站名，
// 而 config.js 本就是全部頁面共同載入的檔案。
...
function openProject(ringIndex) {
    ...
    setTimeout(() => {
        ...
        window.location.href = PROJECT_URL(PROJECTS[ringIndex].slug,
...
//  站名：亦滅 + 逐字透鏡
//  Implementation now lives in shared/wordmark.js (loaded before this
//  file — see index.html) so it is byte-identical across the homepage
//  and all project-XX pages. Bootstrapped once near the end of this
//  file via initWordmark('wordmark', SITE_NAME).
//  實作已移至 shared/wordmark.js（本檔之前載入，見 index.html），
//  使首頁與每個 project-XX 頁面共用同一份程式碼。於本檔尾端以
//  initWordmark('wordmark', SITE_NAME) 綁定一次。
// ---------------------------------------------------------------
